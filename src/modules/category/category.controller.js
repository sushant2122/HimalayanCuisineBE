...
class CategoryController {
        ...
        try {
            ...
            if (req.query.search) {
                filter = {
                    ...filter,
                    [Op.or]: [
                        { category_name: { [Op.iLike]: `%${req.query.search}%` } }, // Case-insensitive LIKE search
                    ]
                };
            }
            // Fetch the list and total count of banners
            const { list, total } = await categorySvc.listAllByFilter({ limit, offset, filter });
    ...
    store = async (req, res, next) => {
        try {
            ...
            res.json({
                ...
                status: "CATEGORY_CREATION_SUCCESS"
            });
    ...
    show = async (req, res, next) => {
        try {
            ...
            const category = await categorySvc.getSingleCategoryData({ category_id: id });
            res.json({
                result: category,
                meta: null,
                message: "Category details.",
                status: "CATEGORY_FOUND"
            });
        } catch (exception) {
            next(exception)
        }
    ...
    update = async (req, res, next) => {
        try {
            ...
            const id = req.params.id;
            const data = req.body;
            const category = await categorySvc.updateCategory(id, data);
            res.json({
                result: category,
                meta: null,
    ...
    remove = async (req, res, next) => {
        try {
            const id = req.params.id;
            const response = await categorySvc.deleteCategoryById(id);
            res.json({
                result: response,
                meta: null,
    ...
    listForHome = async (req, res, next) => {
        try {
            const category = await categorySvc.listAllByFilter({
                limit: 100,
                offset: 0,
                filter
            }
            ...
            res.json({
                result: category,
                meta: null,
                message: "Category for home display.",
